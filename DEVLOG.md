...
- Сделано: добавлен встроенный режим обучения из 3 шагов с подсветкой целевого placement и отдельным финалом без влияния на обычный прогресс.
- Сделано: прохождение tutorial теперь сохраняется в `tutorialCompleted`, а кнопка в меню переключается на повторное прохождение.
- Сделано: из активного pipeline выведены `crazygames`, `poki`, `itchio` и `newgrounds`; поддерживаемые таргеты теперь `generic`, `yandex`, `vkplay`, `rustore`.
- Осталось: ручной smoke tutorial/rewarded сценариев в реальных окружениях Yandex, VK Play и Rustore.
- Риски/Долги: исторические записи про старые платформы сохранены в devlog как контекст миграции.
