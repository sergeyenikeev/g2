...
## Текущее состояние
- Юнит-тесты: актуальный прогон должен быть зелёным; сейчас в наборе 56 тестов, включая tutorial flow, нормализацию прогресса и tracking-события bridge.
...
- Проверка, что после прохождения tutorial кнопка меняется на `Replay Tutorial` / `Повторить обучение`.
