...
Legacy-режимы `crazygames`, `poki`, `itchio` и `newgrounds` выведены из поддержки и больше не входят в build/dev pipeline.
...
Поддерживаемые платформенные режимы:
...
Сборка с моками для всех поддерживаемых платформ:
